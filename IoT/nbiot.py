...
##5 méthodes d’Ensemble Learning (Bagging, Stacking, Voting, Blending,RF) sur 9 datasets différents, calcule les métriques, et génère une image JPG pour chaque dataset.
"""
...
"""## 5 méthodes single Learning (Decision Tree (DT),Deep Neural Network (DNN),AdaBoost (ADA),Support Vector Machine (SVM),Multi-layer Perceptron (MLP)) sur 9 datasets différents, calcule les métriques, et génère une image JPG pour chaque dataset."""
...
# ============================
#      ********Définition du kernel Euclidien********
...
for i, csv_file in enumerate(csv_files):
    ...
    print(f"\n===== SVM Euclidean (RBF) — Dataset {i+1}/9 =====")
...
for i, csv_file in enumerate(csv_files):
    ...
    print(f"\n===== SVM Minkowski p=3 — Dataset {i+1}/9 =====")
...
"""## SGDClassifier (une version très rapide d’un SVM linéaire)"""
...
for i, csv_file in enumerate(csv_files):
    ...
    # Metrics with zero_division=0 → no warnings
...
# 🔥 IMPORTANT : Remplace ci-dessous par le chemin correct trouvé avec ls !
csv_files = [
    '/content/drive/MyDrive/device1_top_20_features.csv',
    '/content/drive/MyDrive/device2_top_20_features.csv',
    '/content/drive/MyDrive/device3_top_20_features.csv',
...
for i, csv_file in enumerate(csv_files, start=4):
    ...
    save_lime_plot(rf_exp, f"Device {i} – Random Forest LIME", f"device{i}_rf_lime.jpg")
    save_lime_plot(dt_exp, f"Device {i} – Decision Tree LIME", f"device{i}_dt_lime.jpg")
    save_lime_plot(ada_exp, f"Device {i} – AdaBoost LIME", f"device{i}_ada_lime.jpg")

    print(f"   ✔ LIME images saved for device {i}")
...
for i, csv_file in enumerate(csv_files):
    ...
    print(f"\n🔎 Processing device {i+1}: {csv_file}")
    ...
    # ------- ⚡ MODELS (parallelised & light AdaBoost) --------
    ...
    # ------- ⚡ CEM ON SMALL BATCH (2000 rows only) --------
    ...
    save_plot(rf_imp, f'Device {i+1} – RF CEM (FAST)', f'device{i+1}_rf_cem_fast.jpg')
    save_plot(dt_imp, f'Device {i+1} – DT CEM (FAST)', f'device{i+1}_dt_cem_fast.jpg')
    save_plot(ada_imp, f'Device {i+1} – AdaBoost CEM (FAST)', f'device{i+1}_ada_cem_fast.jpg')

    print(f"   ✔ FAST CEM images saved for device {i+1}")
...
for i, csv_file in enumerate(csv_files):
    ...
    print("   ✔ Models trained (FAST MODE)")
    ...
    save_plot(rf_imp,  f"Device {i+1} – RF PFI (FAST)",  f"device{i+1}_rf_pfi_fast.jpg")
    save_plot(dt_imp,  f"Device {i+1} – DT PFI (FAST)",  f"device{i+1}_dt_pfi_fast.jpg")
    save_plot(ada_imp, f"Device {i+1} – AdaBoost PFI (FAST)", f"device{i+1}_ada_pfi_fast.jpg")

    print(f"   ✔ FAST PFI images saved for device {i+1}")
...
for i, csv_file in enumerate(csv_files):
    ...
    save_shap(shap_rf, X_sample, f"Device {i+1} – RF SHAP",  f"device{i+1}_rf_shap.jpg")
    save_shap(shap_dt, X_sample, f"Device {i+1} – DT SHAP",  f"device{i+1}_dt_shap.jpg")

    # === SHAP AdaBoost (KernelExplainer) ===
    print("   ⚠ AdaBoost not supported by TreeExplainer → Using KernelExplainer")

    # on prend un plus petit échantillon pour ne pas exploser le temps
    ...
    save_shap(shap_ada, X_ada, f"Device {i+1} – AdaBoost SHAP", f"device{i+1}_ada_shap.jpg")

    print(f"   ✔ SHAP images saved for device {i+1}")
...
for i, csv_file in enumerate(csv_files):
    ...
    for feature in X.columns:
        save_ale(rf, feature,
                 f"Device {i+1} – RF ALE – {feature}",
                 f"device{i+1}_rf_ale_{feature}.jpg")

        save_ale(dt, feature,
                 f"Device {i+1} – DT ALE – {feature}",
                 f"device{i+1}_dt_ale_{feature}.jpg")

        save_ale(ada, feature,
                 f"Device {i+1} – ADA ALE – {feature}",
                 f"device{i+1}_ada_ale_{feature}.jpg")

    print(f"   ✔ ALE images saved for device {i+1}")
...
# =========================================================
# 🚀 High-Accuracy QSVM (0.7 – 0.85) – Optimized One Cell
# =========================================================
...
pca = PCA(n_components=6)   # more info preserved → better accuracy
...
def feature_map(x):
    for i in range(6):
        qml.RX(x[i], wires=i)
        qml.RZ(x[i]**2, wires=i)     # ✔ non-linear term: big improvement
